...
//        StopWatch stopWatch = new StopWatch();
        ...
        if(!Arrays.areEquals(arr1, correntArray)) {
            errorMsg += "Bubble";
        }
        if(!Arrays.areEquals(arr2, correntArray)) {
            errorMsg += (errorMsg.length() > 0 ? ", " : "") + "Selection";
        }
        if(!Arrays.areEquals(arr3, correntArray)) {
            errorMsg += (errorMsg.length() > 0 ? ", " : "") + "Insertion";
        }
        if(!Arrays.areEquals(arr4, correntArray)) {
            errorMsg += (errorMsg.length() > 0 ? ", " : "") + "Merge";
        }
        if(!Arrays.areEquals(arr5, correntArray)) {
            errorMsg += (errorMsg.length() > 0 ? ", " : "") + "Quick";
        }
        if(!Arrays.areEquals(arr6, correntArray)) {
            errorMsg += (errorMsg.length() > 0 ? ", " : "") + "Count";
        }
        if(!Arrays.areEquals(arr7, correntArray)) {
            errorMsg += (errorMsg.length() > 0 ? ", " : "") + "Bucket";
        }
        if(!Arrays.areEquals(arr8, correntArray)) {
            errorMsg += (errorMsg.length() > 0 ? ", " : "") + "Radix";
        }
        if(!Arrays.areEquals(arr9, correntArray)) {
